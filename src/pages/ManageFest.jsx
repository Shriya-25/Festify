      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="glass-container p-4 sm:p-6 border border-white/10">
              ...
              {events.length === 0 ? (
                <div className="text-center py-6 sm:py-8">
                  ...
                  <p className="text-sm sm:text-base text-gray-400 mb-4">No events yet</p>
                  <Link
                    to={`/fest/${festId}/create-event`}
                ...
                <div className="space-y-3 sm:space-y-4">
                  {events.map(event => (
                    <div
                      key={event.id}
                      className="rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition overflow-hidden"
                    >
                      <div className="flex flex-col sm:flex-row items-start">
                        {/* Event Banner Thumbnail */}
                        {event.bannerUrl && (
                          <div className="w-full sm:w-24 md:w-32 h-32 sm:h-24 md:h-32 flex-shrink-0 bg-gray-800/50 overflow-hidden">
                            <img 
                              src={event.bannerUrl} 
                              alt={event.eventName}
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1.5 sm:gap-2 mt-1 flex-wrap">
                              ...
                              {event.status === 'approved' ? (
                                <span className="inline-block px-1.5 sm:px-2 py-0.5 sm:py-1 bg-green-500/20 text-green-400 text-[10px] sm:text-xs font-semibold rounded-full border border-green-500/30">
                                  Approved
                                </span>
                              ) : event.status === 'rejected' ? (
                                <span className="inline-block px-1.5 sm:px-2 py-0.5 sm:py-1 bg-red-500/20 text-red-400 text-[10px] sm:text-xs font-semibold rounded-full border border-red-500/30">
                                  Rejected
                                </span>
                              ) : event.status === 'changes_requested' ? (
                                <span className="inline-block px-1.5 sm:px-2 py-0.5 sm:py-1 bg-orange-500/20 text-orange-400 text-[10px] sm:text-xs font-semibold rounded-full border border-orange-500/30">
                                  Changes Requested
                                </span>
                              ) : (
                                <span className="inline-block px-1.5 sm:px-2 py-0.5 sm:py-1 bg-yellow-500/20 text-yellow-400 text-[10px] sm:text-xs font-semibold rounded-full border border-yellow-500/30">
                                  Pending
                                </span>
                              )}
                              {/* Registration Status */}
                              {isRegistrationOpen(event) ? (
                                <span className="inline-block px-1.5 sm:px-2 py-0.5 sm:py-1 bg-blue-500/20 text-blue-400 text-[10px] sm:text-xs rounded-full border border-blue-500/30">
                                  Open
                                </span>
                              ) : (
                                <span className="inline-block px-1.5 sm:px-2 py-0.5 sm:py-1 bg-gray-500/20 text-gray-400 text-[10px] sm:text-xs rounded-full border border-gray-500/30">
                                  Closed
                                </span>
                              )}
                            </div>
                            ...
                            {event.isPaid && event.participantCount > 0 && (
                              <p className="text-xs sm:text-sm font-semibold text-green-400 mt-1">
                                Funds: ₹{calculateEventFunds(event)}
                              </p>
                            )}
                            {event.adminComments && (
                              <div className="mt-2 bg-yellow-500/10 p-2 rounded-lg text-[10px] sm:text-xs text-yellow-400 border border-yellow-500/20">
                      ...
                      <div className="flex gap-2 p-2 sm:p-3 md:p-4 pt-2 border-t border-white/10">
                        <button
                          onClick={() => handleEventClick(event)}
                        ...
                        <Link
                          ...
                          className="flex-1 text-xs sm:text-sm py-1.5 sm:py-2 px-2 sm:px-3 rounded-lg sm:rounded-xl bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 text-center border border-blue-500/30 transition-colors"
                        >
                          Edit
                        </Link>
                      </div>
          ...
          <div className="lg:col-span-2">
            {!selectedEvent ? (
              <div className="glass-container p-6 sm:p-8 md:p-12 text-center border border-white/10">
              ...
              <div className="glass-container p-4 sm:p-6 border border-white/10">
                <div className="flex flex-col sm:flex-row sm:flex-wrap justify-between items-start gap-3 sm:gap-4 mb-4 sm:mb-6">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
                      <p className="text-xs sm:text-sm text-gray-300">
                        Total: <span className="font-semibold text-white">{registrations.length}</span> students
                      </p>
                ...
                {loadingRegistrations ? (
                  <div className="text-center py-8 sm:py-12">
                    ...
                    <p className="text-sm sm:text-base text-gray-300 mt-4">Loading registrations...</p>
                  </div>
                ) : registrations.length === 0 ? (
                  <div className="text-center py-8 sm:py-12">
                    <svg className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <p className="text-sm sm:text-base text-gray-400">No registrations yet</p>
                  </div>
                ) : (
                  <>
                    {/* Search */}
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      {filteredRegistrations.map(reg => (
                        <div
                          key={reg.id}
                          className="block p-3 sm:p-4 border border-white/10 rounded-xl sm:rounded-2xl bg-white/5 hover:bg-white/10 transition-colors"
                        >
                          <div className="flex flex-col sm:flex-row justify-between items-start gap-2 sm:gap-0">
                            <div className="flex-1">
                              ...
                              <p className="text-xs sm:text-sm text-gray-300">{reg.email}</p>
                              <p className="text-xs sm:text-sm text-gray-300">
                                {reg.college} {reg.branch && `• ${reg.branch}`} {reg.year && `• ${reg.year}`}
                              </p>
                              {reg.phone && (
                                <p className="text-xs sm:text-sm text-gray-500 mt-1">📱 {reg.phone}</p>
                              )}
                              ...
                              {selectedEvent.isPaid && reg.paymentProof && (
                                <div className="mt-2 sm:mt-3 space-y-2">
                                  <div className="flex items-center flex-wrap gap-1.5 sm:gap-2">
                                    <span className="text-xs sm:text-sm font-medium text-gray-300">Payment Status:</span>
                                    ...
                                    {reg.paymentProof.paymentMethod === 'razorpay' && (
                                      <span className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 bg-purple-500/20 text-purple-400 rounded border border-purple-500/30">
                                        Razorpay
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  {reg.couponUsed && reg.couponUsed.code && (
                                    <div className="bg-green-500/10 border border-green-500/30 rounded p-2 mt-2">
                                      <p className="text-[10px] sm:text-xs font-semibold text-green-400">🎟️ Coupon Applied: {reg.couponUsed.code}</p>
                                      <div className="flex flex-wrap justify-between gap-2 text-[10px] sm:text-xs text-gray-300 mt-1">
                                        <span>Original: ₹{reg.couponUsed.originalAmount}</span>
                                        <span className="text-green-400 font-semibold">{reg.couponUsed.discountPercent}% OFF</span>
                                        <span className="font-semibold">Paid: ₹{reg.couponUsed.finalAmount}</span>
                                      </div>
                                      <p className="text-[10px] sm:text-xs text-green-400 mt-1">Saved: ₹{reg.couponUsed.discountAmount.toFixed(2)}</p>
                                    </div>
                                  )}
        ...
        {showPaymentProof && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
            <div className="glass-container border border-white/10 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 glass-container border-b border-white/10 p-4 sm:p-6 flex justify-between items-center">
              ...
              <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                <div>
                  <h3 className="text-sm sm:text-base font-semibold text-white mb-2 sm:mb-3">Student Details</h3>
                  <div className="bg-white/5 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-white/10 space-y-2">
                    <p className="text-xs sm:text-sm text-gray-300"><strong className="text-white">Name:</strong> {showPaymentProof.name}</p>
                    <p className="text-xs sm:text-sm text-gray-300"><strong className="text-white">Email:</strong> {showPaymentProof.email}</p>
                    <p className="text-xs sm:text-sm text-gray-300"><strong className="text-white">Phone:</strong> {showPaymentProof.phone}</p>
                  </div>
                </div>
                ...
                {showPaymentProof.paymentProof && (
                  <>
                    <div>
                      <h3 className="text-sm sm:text-base font-semibold text-white mb-2 sm:mb-3">Transaction Details</h3>
                      <div className="bg-white/5 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-white/10 space-y-2">
                        <p className="text-xs sm:text-sm text-gray-300"><strong className="text-white">Transaction ID:</strong> {showPaymentProof.paymentProof.transactionId}</p>
                        <p className="text-xs sm:text-sm text-gray-300">
                        ...
                        <p className="text-xs sm:text-sm text-gray-300"><strong className="text-white">Submitted:</strong> {new Date(showPaymentProof.paymentProof.submittedAt).toLocaleString()}</p>
                    ...
                    <div>
                      <h3 className="text-sm sm:text-base font-semibold text-white mb-2 sm:mb-3">Payment Screenshot</h3>
                      <div className="border border-white/10 rounded-lg sm:rounded-xl p-2 sm:p-4 bg-white/5">
                        <img
